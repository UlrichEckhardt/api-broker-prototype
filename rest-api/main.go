...
func Serve(ctx context.Context, store events.EventStore, port uint) error {
	...
	e.GET("/request/:external_uuid", func(c echo.Context) error {
		// extract external UUID from the path
		external_uuid := uuid.FromStringOrNil(c.Param("external_uuid"))
		if external_uuid == uuid.Nil {
			return c.String(http.StatusBadRequest, "bad request")
		}
		e.Logger.Debug("external_uuid=", external_uuid.String())

		id, err := store.ResolveUUID(c.Request().Context(), external_uuid)
		if err != nil {
			return c.String(http.StatusNotFound, "failed to resolve external UUID")
		}
		e.Logger.Debug("resolved id=", id)

		envelope, err := store.RetrieveOne(c.Request().Context(), id)
		if err != nil {
